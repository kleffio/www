        ...
        {SIMPLE_NAV_LINKS.map((link) => (
          <NavigationMenuItem key={link.href}>
            <NavigationMenuLink asChild>
              ...
              >
                {/* NEW: simple top-level links localized */}
                {link.label}
              </UnderlineLink>
            </NavigationMenuLink>
          </NavigationMenuItem>
...
const ProductMegaMenu = memo(
  ({ section, t }: { section: MegaMenuSection; t: any }) => {
    const p = t.product;
      ...
      <div className="grid grid-cols-[minmax(0,2.2fr)_minmax(0,2.1fr)] gap-8 py-5 md:w-[680px] lg:w-[840px]">
            ...
            {section.items.map((item) => {
              ...
              return (
                <NavigationMenuLink asChild key={item.label}>
                  <Link
                    to={item.href}
                    className="group flex items-start gap-3 rounded-lg px-3 py-2.5 hover:bg-white/4 transition-colors duration-150"
                  >
        ...
        <div className="flex flex-col gap-4">
          ...
          <InfoCard
            ...
            items={[
              p.compare.vs_vercel,
              p.compare.vs_amplify,
              p.compare.vs_kubernetes
            ]}
          />
        </div>
      </div>
...
// ---------------------------------------------------------
// DEFAULT MENU (Developers, Solutions, etc.)
// ---------------------------------------------------------

const DefaultMegaMenu = memo(
  ({ section, t }: { section: MegaMenuSection; t: any }) => {
          ...
          {section.items.map((item) => {
            ...
            return (
              <NavigationMenuLink asChild key={item.label}>
                <Link
                  to={item.href}
                  className="group relative flex items-start gap-3 rounded-xl border border-white/6 bg-black/30 px-3 py-2.5 hover:bg-white/5 transition-colors duration-150"
                >
...
// ---------------------------------------------------------
// Sub-components (unchanged except using localized props)
// ---------------------------------------------------------
...
function MenuItemContent({ label, description, size = "md" }: any) {
...
function InfoCard({ title, heading, description, items }: any) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      {description && <p className="text-muted-foreground mt-1 text-[11px] leading-snug">{description}</p>}
