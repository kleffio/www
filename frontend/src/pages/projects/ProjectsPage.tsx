...
export function ProjectsPage() {
  ...
  async function loadProjects() {
    try {
      setLoading(true);
      setError(null);
      const res = await axiosInstance.get<Project[]>("/api/v1/projects");
          ...
          <SoftPanel>
            <div className="py-10 text-center">
              ...
              <Button
                size="sm"
                onClick={() => setIsModalOpen(true)}
                className="mt-4 bg-gradient-kleff rounded-full px-4 py-1.5 text-xs font-semibold text-black shadow-md shadow-black/40 hover:brightness-110"
              >
                          ...
                          <TableCell className="text-neutral-300">
                            {p.repositoryUrl ? (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="text-xs text-kleff-gold hover:underline"
                              >
                                {p.repositoryUrl}
                              </a>
                            ) : (
