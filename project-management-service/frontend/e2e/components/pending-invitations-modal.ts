...
export class PendingInvitationsModal extends BaseComponent {
  ...
  async open() {
    ...
    const bellButton = this.page
      .locator("button")
      .filter({ has: this.page.locator("svg.lucide-bell") });
  ...
  async acceptInvitation(projectName: string) {
    ...
    const acceptButton = row
      ...
      .or(row.locator("button").filter({ has: this.page.locator("svg.lucide-check") }));
  ...
  async rejectInvitation(projectName: string) {
    ...
    const rejectButton = row
      ...
      .or(row.locator("button").filter({ has: this.page.locator("svg.lucide-x") }));
  ...
  async expectNoInvitations() {
    await expect(this.modal()).toBeVisible();
    await expect(this.page.getByText(/no.*invitation|no new notification/i)).toBeVisible({
      timeout: 10_000
    });
  }
